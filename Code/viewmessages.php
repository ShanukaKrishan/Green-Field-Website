...
<h2 style="text-align: center;">Messages from keels</h2>
...
<?php
	...
	$sql = "SELECT * FROM keelsmsg Where username='$username'";

	
	$results = mysqli_query($db,$sql);
while ($row = mysqli_fetch_array($results)) {
  echo "<h3>Product Name : ".$row['name']."</h3>";
		echo "<input disabled type='text' value='".$row['messagekeels']."'>";
    echo "<p>from ".$row['sendersuname']."</p>";
    echo "<center>";
    echo "<a id='replybtn' href='messagekeels.php?suname=".$row['sendersuname']."&name=".$row['name']."&username=".$_SESSION["username"]."'>Send a Reply to the message</a>" ;
    echo "<br>";echo "<br>";
    echo "<p> -----------------------------------------------------------------------------------------------------------------</p>";
echo "</center>";
